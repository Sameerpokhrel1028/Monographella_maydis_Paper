...
clean_gspecies <- function(x) {
  ...
  x <- sub("^Phyllachora[ _\\.]*maydis.*", "P.maydis", x)
  x <- sub("^Mmaydis.*", "M.maydis", x)
  x <- sub("^Monographella[ _\\.]*maydis.*", "M.maydis", x)
...
d <- as.dist(mat)
hc <- hclust(d, method = "average")   # "average" is a good default (UPGMA)


# Color scale: Green (close) -> Red (far)
# Auto-range based on observed distances
vals <- as.vector(mat)
